...
class TestThinPack(PackTests):
    ...
    def test_get_raw(self):
        self.assertRaises(
            KeyError, self.make_pack(False).get_raw, self.blobs[b'foo1234'].id)
        self.assertEqual(
            (3, b'foo1234'),
            self.make_pack(True).get_raw(self.blobs[b'foo1234'].id))
