...
class CreateRepositoryTests(TestCase):
    ...
    def _check_repo_contents(self, repo, expect_bare):
        ...
        barestr = b'bare = ' + bytes(expect_bare).lower()
...
class RepositoryTests(TestCase):
    ...
    def test_get_description(self):
        r = self._repo = open_repo('a.git')
        f = open(os.path.join(r.path, 'description'), 'wb')
        try:
            f.write(b"Some description")
        finally:
            f.close()
        self.assertEquals(b"Some description", r.get_description())
    ...
    def test_get_peeled(self):
        ...
        self.assertNotEqual(r[tag_sha].sha().hexdigest().encode('ascii'), r.head())
